...
class EcommerceConnector(models.Model):
    ...
    def _get_shipping_contact(self, partner, values, ecommerce_connection):
        ...
        if ecommerce_connection.shipping_address_search_rule == 'ecommerce_id':
            ...
            if ecommerce_partner_id and ecommerce_partner_id.partner_id.type == 'delivery' and ecommerce_partner_id.partner_id.parent_id == partner:
                partner_id = ecommerce_partner_id.partner_id
        ...
        if not partner_id:
            partner_id = self._create_new_shipping_partner(values, partner, ecommerce_connection)
        else:
    ...
    def _get_invoice_contact(self, partner, values, ecommerce_connection):
        ...
        if ecommerce_connection.invoice_address_search_rule == 'ecommerce_id':
            ...
            if ecommerce_partner_id and ecommerce_partner_id.partner_id.type == 'invoice' and ecommerce_partner_id.partner_id.parent_id == partner:
                partner_id = ecommerce_partner_id.partner_id
        ...
        if not partner_id:
            partner_id = self._create_new_invoice_partner(values, partner, ecommerce_connection)
        else:
            partner_id = partner_id[0]
        return partner_id
    ...
    def _get_order_line(self, fiscal_position, line, ecommerce_connection):
        ...
        taxes = fiscal_position.with_company(company).map_tax(product_id.taxes_id).filtered(
    ...
    def _get_order_lines(self, fiscal_position_id, values, ecommerce_connection):
        ...
        for line in values.get('lines'):
            order_lines.append((0, 0, self._get_order_line(fiscal_position_id, line, ecommerce_connection)))
    ...
    def _get_credit_note_lines(self, lines, ecommerce_connection):
        ...
        credit_note_lines = []
        for line in lines:
            credit_note_lines.append((0, 0, self._get_credit_note_line(line, ecommerce_connection)))
        return credit_note_lines

    def _check_mandatory_fields(self, errors, values, company_id, ecommerce_connection, excluded_fields=[]):
        ...
        if 'customer.id' not in excluded_fields and not values.get('customer').get('id'):
        ...
        if 'customer.email' not in excluded_fields and not values.get('customer').get('email'):
        ...
        if 'shippingAddress.id' not in excluded_fields and not values.get('shippingAddress').get('id'):
            errors = self._write_errors(errors, "Ecommerce shipping address ID is missing.")
        if 'shippingAddress.firstname' not in excluded_fields and not values.get('shippingAddress').get('firstname'):
            errors = self._write_errors(errors, "Shipping address firstname is missing.")
        if 'shippingAddress.countryCode' not in excluded_fields and not values.get('shippingAddress').get('countryCode'):
            errors = self._write_errors(errors, "Shipping address country code is missing.")
        if 'shippingAddress.street' not in excluded_fields and not values.get('shippingAddress').get('street'):
            errors = self._write_errors(errors, "Shipping address street is missing.")
        if 'shippingAddress.postcode' not in excluded_fields and not values.get('shippingAddress').get('postcode'):
        ...
        if 'billingAddress.id' not in excluded_fields and not values.get('billingAddress').get('id'):
            errors = self._write_errors(errors, "Ecommerce billing address ID is missing.")
        if 'billingAddress.firstname' not in excluded_fields and not values.get('billingAddress').get('firstname'):
            errors = self._write_errors(errors, "Billing address firstname is missing.")
        if 'billingAddress.countryCode' not in excluded_fields and not values.get('billingAddress').get('countryCode'):
            errors = self._write_errors(errors, "Billing address country code is missing.")
        if 'billingAddress.street' not in excluded_fields and not values.get('billingAddress').get('street'):
            errors = self._write_errors(errors, "Billing address street is missing.")
        if 'billingAddress.postcode' not in excluded_fields and not values.get('billingAddress').get('postcode'):
        ...
        if any(not line.get('productName') or \
                ...
                ('line.productSku' not in excluded_fields and not line.get('productSku')) or \
                ('line.productBarcode' not in excluded_fields and not line.get('productBarcode')) or \
                ...
                ('line.taxes' not in excluded_fields and not line.get('taxes')) for line in values.get('lines')):
        ...
        for line in values.get('lines'):
            if any(('tax.taxId' not in excluded_fields and tax.get('taxId') is None) or \
                    ('tax.taxName' not in excluded_fields and not tax.get('taxName')) \
        ...
        if ecommerce_connection.product_search_rule == 'barcode' and \
        ...
        if ecommerce_connection.contact_search_rule == 'vat' and \
        ...
        if ecommerce_connection.invoice_address_search_rule == 'ecommerce_id' and \
                not values.get('billingAddress').get('id'):
            errors = self._write_errors(
                errors,
                "Billing address is searched by id but it is not provided."
            )
        if ecommerce_connection.invoice_address_search_rule == 'email' and \
        ...
        if ecommerce_connection.shipping_address_search_rule == 'ecommerce_id' and \
                not values.get('shippingAddress').get('id'):
            errors = self._write_errors(
                errors,
                "Shipping address is searched by id but it is not provided."
            )
        if ecommerce_connection.shipping_address_search_rule == 'email' and \
    ...
    def _check_payments(self, errors, values):
        ...
        if values.get('payments'):
            ...
            amount_paid = sum(float(payment.get('unitPrice')) for payment in values.get('payments'))
    ...
    def _check_shipping_lines(self, values, move, errors):
        ...
        if values.get('shipments'):
            for line in values.get('shipments'):
                ...
                if float_compare(float(line.get('unitPrice')), compare_val, precision_digits=2) != 0:
                    errors = self._write_errors(
                        errors,
    ...
    def _check_invoice_payments(self, values, move, errors):
        ...
        if payments:
            ...
            for payment in payments:
                ...
                elif payment_id and float_compare(float(payment_id.amount_total_signed), float(payment.get('unitPrice')), precision_digits=2) != 0:
                    errors = self._write_errors(
                        errors,
    ...
    def _create_new_product(self, template, line, ecommerce_connection):
        ...
        if template.product_variant_count > 1:
            product_id = template.product_variant_ids.filtered(
                lambda a: attributes == a.product_template_attribute_value_ids.mapped('product_attribute_value_id')
            )
        ...
        if product_id:
            ...
            if product_vals:
                ...
                product_id.with_company(ecommerce_connection.company_id.id).write(product_vals)
        return product_id
        ...
        for line in lines:
            ...
            if not product_id:
                if line.get('productBarcode') and self.env['product.product'].search([
                    ...
                    errors = self._write_errors(
                        errors,
                        "Product with barcode %s already in the system." % line.get('productBarcode')
                    )
                if not errors:
                    ...
                    if ecommerce_product_template:
                        product_template_id = ecommerce_product_template.product_template_id
                    if not product_template_id:
                        product_template_id = self._create_new_product_template(line, company, ecommerce_connection)
                    if line.get('variants'):
                        self._create_new_product(product_template_id, line, ecommerce_connection)
    ...
    def _create_sale(self, connector_call, values, errors, ecommerce_connection=False, payment_errors=False, order_id=False, move_id=False):
    ...
    def external_create_sale(self, values):
        ...
        return self._create_sale(connector_call, values, errors, ecommerce_connection_id, payment_errors, order_id, move_id)
        ...
        if not errors:
            ...
            if not errors:
                ...
                if not ecommerce_connection:
                    errors = self._write_errors(errors, "Ecommerce Connection is missing.")
                if not errors:
                    invoice_id = self.env['account.move'].with_company(company).search([
                        ('ecommerce_id', '=', int(values.get('invoiceId'))),
                        ('ecommerce_connector_id', '=', int(ecommerce_connection))
                    ])
                    if not invoice_id:
