...
class ProductManagementConnector(models.Model):
    ...
    def _check_create_mandatory_fields(
        ...
        if not values.get("productName"):
            errors = self._write_errors(errors, "Product name in product not provided.")
        if not values.get("productTemplateId"):
            errors = self._write_errors(
                errors, "Product template ID in product not provided."
            )
        if not values.get("productId"):
            errors = self._write_errors(errors, "Product ID in product not provided.")
        if not values.get("productTaxCompany"):
            errors = self._write_errors(errors, "Product tax in product not provided.")
        if not values.get("productType"):
            errors = self._write_errors(errors, "Product type in product not provided.")
        return errors

    def _check_update_mandatory_fields(
        self, errors, values, company, ecommerce_connection
    ):
        """Returns a string with the errors
        ...
        """
        if (
            ecommerce_connection.product_search_rule == "ecommerce_id"
            and not values.get("productId")
        ):
            errors = self._write_errors(errors, "Product ID in product not provided.")
        if ecommerce_connection.product_search_rule == "sku" and not values.get(
            "productSku"
        ):
            errors = self._write_errors(errors, "Product SKU in product not provided.")
        if ecommerce_connection.product_search_rule == "barcode" and not values.get(
            "productBarcode"
        ):
            errors = self._write_errors(
                errors, "Product barcode in product not provided."
            )
        return errors
    ...
    def _get_product_template(self, product_vals, company, ecommerce_connection):
        ...
        if not ecommerce_product_template:
            product_template = self.env[
                "ecommerce.connector"
            ]._create_new_product_template(product_vals, company, ecommerce_connection)
        else:
    ...
    def _check_connection_values(self, values, errors):
        ...
        if not company_id:
            ...
            errors = self._write_errors(
                errors, "Company does not accept the external call."
            )
    ...
    def _get_update_vals(self, values, ecommerce_connection, company):
        ...
        elif ecommerce_connection.product_search_rule == "barcode" and vals.get(
            "barcode"
        ):
            del vals["barcode"]
        return vals
    ...
    def _update_additional_fields(self, values, company, product, errors):
        ...
        if values.get("productTaxCompany"):
            tax_id = self.env["ecommerce.connector"]._get_tax_by_country(
                values.get("productTaxCompany"), company.country_id, company
            )
            if not tax_id:
                errors = self._write_errors(errors, "Taxes not found")
            if not errors:
                current_taxes = product.taxes_id.filtered(
                    lambda a: a.company_id.id == int(values.get("companyId"))
                )
                for tax in current_taxes:
                    product.write({"taxes_id": [(3, tax.id)]})
                if tax_id:
                    product.write(
                        {
    ...
    def external_create_product(self, values):
        errors = ""
        errors, ecommerce_connection, company = self._check_connection_values(
            values, errors
        )
        if errors:
            return self._create_answer(
                errors, values, "create_product", ecommerce_connection
            )
        errors = self._check_create_mandatory_fields(
            errors, values, company, ecommerce_connection
        )
        if errors:
            return self._create_answer(
                errors, values, "create_product", ecommerce_connection
        ...
        product_template = self._get_product_template(
            values, company, ecommerce_connection
        )
        if values.get("variants"):
            ...
            _logger.info(f"Product with ID {product.id} created.")
        return self._create_answer(
            errors, values, "create_product", ecommerce_connection
        )
    ...
    def external_update_product(self, values):
        ...
        if errors:
            return self._create_answer(
                errors, values, "update_product", ecommerce_connection
            )
        errors = self._check_update_mandatory_fields(
            errors, values, company, ecommerce_connection
        )
        if errors:
            return self._create_answer(
        ...
        product = self.env["ecommerce.connector"]._find_product(
            values, ecommerce_connection
        )
        if not product:
            errors = self._write_errors(errors, "Product not found.")
            return self._create_answer(
                errors, values, "update_product", ecommerce_connection
            )
        # We use with_company as standard_price is company dependent.
        product.with_company(company).with_context(
            lang=ecommerce_connection.lang
        ).write(self._get_update_vals(values, ecommerce_connection, company))
        errors = self._update_additional_fields(values, company, product, errors)
        _logger.info(f"Product with ID {product.id} updated.")
        return self._create_answer(
            errors, values, "update_product", ecommerce_connection
        )
