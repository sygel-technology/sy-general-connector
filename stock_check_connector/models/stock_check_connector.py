...
SEARCH_KEYS = ["odooId", "ecommerceId", "sku", "barcode"]
...
class StockCheckConnector(models.Model):
    ...
    def _lot_vals(self, product, lot_serial_search, date=False):
        ...
        if lot_serial_search == "available":
            lot_ids = lot_ids.filtered(lambda a: a.product_qty > 0.0)
        if lot_serial_search == "all" and date:
            lot_ids = lot_ids.filtered(
                lambda a: a.product_qty > 0.0
                or a.create_date >= datetime.strptime(date, "%Y-%m-%d")
            )
        for lot in lot_ids:
            lot_vals.append(
                {"name": lot.name, "internalRef": lot.ref, "quantity": lot.product_qty}
            )
        return lot_vals
    ...
    def _product_vals(
        ...
        lot_serial_search=False,
        lot_serial_date=False,
    ):
        ...
        if lot_serial_search and product.tracking in ["serial", "lot"]:
            vals["lot_serial"] = self._lot_vals(
                product, lot_serial_search, lot_serial_date
            )
        ...
        if not values.get("companyId"):
            ...
            # Check Ecommerce Connection
                ...
                if not values.get("ecommerceId"):
                    errors = self._write_errors(
                        errors, "Ecommerce ID must be provided."
                    )
                else:
        ...
        if values.get("products"):
            for p in values.get("products"):
                if any(not p.get("searchKey") for p in values.get("products")):
                    errors = self._write_errors(
                        errors, "Search Key must be present in all products."
                    )
                elif any(
                    p.get("searchKey") not in SEARCH_KEYS
                    for p in values.get("products")
                ):
                    errors = self._write_errors(
                        errors, "Search Key value is not correct in some products."
                    )
                elif any(not p.get("searchVal") for p in values.get("products")):
                    errors = self._write_errors(
                        errors, "Search Value must be present in all products."
                    )
                if errors:
                    break

        # Check lot/serial values
        if values.get("lotSerialSearch") and values.get("lotSerialSearch") not in [
            "available",
            "all",
        ]:
    ...
    def _find_product(self, search_key, search_val, ecommerce_connection):
        ...
        if search_key == "ecommerceId":
            ...
            domain = [("company_id", "in", [False, ecommerce_connection.company_id.id])]
    ...
    def _external_stock_check(self, values, errors, products, ecommerce_connection):
        if errors:
            ...
            vals = {"status": "OK", "result": result_list}
        return vals
            ...
            if not values.get("products"):
                ...
                for p in values.get("products"):
                    ...
                    if not product:
                        ...
                        products += product
        return self._external_stock_check(
            values, errors, products, ecommerce_connection
        )
